...
df.to_csv(f'{SOFA_PATH}/VASO_CV.csv', index=False)
...
columns = [i[0] for i in mycursor.description]
df = pd.DataFrame(result, columns=columns)
df.to_csv(f'{SOFA_PATH}/VASO_MV.csv', index=False)
...
for j in range(len(TABLES)):
    ...
    columns = [i[0] for i in mycursor.description]
    df = pd.DataFrame(result, columns=columns)
    df.to_csv(f'{SOFA_PATH}/' + NAMES[j] + '.csv', index=False)
...
for j in TABLES2:
    ...
    columns = [i[0] for i in mycursor.description]
    df = pd.DataFrame(result, columns=columns)
    df.to_csv(f'{SOFA_PATH}/' + j + '.csv', index=False)
